...
export default class Tour extends Component {
  ...
  componentWillMount() {
    ...
    ProfileStore.on("show_educations", this.getEducations);
  ...
  componentWillUnmount() {
    ...
    ProfileStore.removeListener("show_educations", this.getEducations);
  ...
  componentDidMount(){
    ...
    MyActions.getEducations(this.$f7route.params['profileId'], this.state.token);
  ...
  getEducations() {
    var educations = ProfileStore.getEducations();
  ...
  createItem(){
    if (this.state.educations) {
      var length = this.state.educations.length;
      let items = []
      for (let i = 0; i < length; i++) {
        items.push(      <List simple-list>
          <ListItem title={dict.school_name}>{this.state.educations[i].school_name}</ListItem>
          <ListItem title={dict.grad_level}>{this.state.educations[i].grad_level}</ListItem>
          <ListItem title={dict.grad_year}>{this.state.educations[i].grad_year}</ListItem>
          <ListItem title={dict.field}>{this.state.educations[i].field}</ListItem>
          <ListItem title={dict.country}>{this.state.educations[i].country}</ListItem>
          <ListItem title={dict.city}>{this.state.educations[i].city}</ListItem>
        </List>);
      }
      return items
    }
